...
LAYER_SRC="layer_linux/src/layer.cpp
    ...
    layer_linux/src/motion/globalmotion_vk.cpp"
...
if command -v qmake6 >/dev/null 2>&1; then
    ...
    if pkg-config --exists Qt6Widgets; then
        # Qt6 dropped QT_INSTALL_LIBEXECDIR, so probe the distro layouts and PATH instead -- and
        # check what each one actually is. A bare `moc` on PATH is Qt5's on any system with both
        # installed, and a Qt5 moc against Qt6 headers fails with "the moc has changed too much"
        # rather than with anything that names the real problem.
        MOC=""
        for c in /usr/lib64/qt6/libexec/moc /usr/lib/qt6/libexec/moc /usr/lib/qt6/moc \
                 "$(command -v moc 2>/dev/null || true)"; do
            if [ -x "$c" ] && "$c" --version 2>/dev/null | grep -q " 6\."; then MOC="$c"; break; fi
        done
        if [ -x "$MOC" ]; then
